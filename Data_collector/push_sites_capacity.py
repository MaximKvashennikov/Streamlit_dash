...
class PushReport(Report):
    ...
    @staticmethod
    def qet_rollout_path():
        """ Получение последнего файла с пролетами из папки загрузок """

        path = '/mnt/pushreports/'
        preview_week = f"{str(datetime.now().year)[-2:]}{three_week()[1][-2:]}"

        return path + f"Reports_{preview_week}" + f"/Push_sites_Rollout_w{preview_week}.xlsx"
    ...
    def show_push_capacity(self, df):
        year = str(datetime.now().year)[-2:]
        start_year = int(year + "00")
        end_year = int(year + "00") + 100

        # df = self.get_report_push_capacity(path=self.qet_capacity_path())
        ...
        fig.add_trace(go.Bar(
            ...
            name=f"План - {df_plan['Количество, шт.'].sum()} шт.",
            text=df_plan['region'],
        ...
        fig.add_trace(go.Bar(
            ...
            name=f"Факт - {df_fact['Количество, шт.'].sum()} шт.",
            text=df_fact['region'],
            texttemplate="%{text}<br> %{y}шт.",
            textfont={'size': 15},
            textposition="inside",
            ...
            buttons_mr.append(dict(method='restyle',
                                   ...
                                   args=[
                                       {'y': [(df_plan[df_plan["macroregion"] == mr])['Количество, шт.'],
                                        ...
                                        'name': [
                                            f"План - {(df_plan[df_plan['macroregion'] == mr])['Количество, шт.'].sum()} шт.",
                                            f"Факт - {(df_fact[df_fact['macroregion'] == mr])['Количество, шт.'].sum()} шт."
                                        ],
                                        'cells': dict(
                                            values=(
                                                df_sites[df_sites["macroregion"] == mr]).transpose().values.tolist(),
                                            fill=dict(
                                                color=self.change_color(df=df_sites, mr_reg=mr,
                                                                        df_filter="macroregion")),
        ...
        buttons_mr.append(dict(method='restyle',
                               ...
                               args=[
                                   {'y': [df_plan['Количество, шт.'],
                                          df_fact['Количество, шт.']],
                                    'x': [df_plan['Site RFS'],
                                          df_fact['Site RFS']],
                                    'text': [df_plan['region'],
                                             df_fact['region']],
                                    'name': [
                                        f"План - {df_plan['Количество, шт.'].sum()} шт.",
                                        f"Факт - {df_fact['Количество, шт.'].sum()} шт."
                                    ],
                                    'cells': dict(
                                        values=df_sites.transpose().values.tolist(),
                                        font=dict(color='black', size=12),
                                        align='left',
                                    )
                                    }
            ...
            buttons_reg.append(dict(method='restyle',
                                    ...
                                    args=[
                                        {'y': [(df_plan[df_plan["region"] == reg])['Количество, шт.'],
                                         ...
                                         'name': [
                                             f"План - {(df_plan[df_plan['region'] == reg])['Количество, шт.'].sum()} шт.",
                                             f"Факт - {(df_fact[df_fact['region'] == reg])['Количество, шт.'].sum()} шт."
                                         ],
                                         'cells': dict(
                                             values=(df_sites[df_sites["region"] == reg]).transpose().values.tolist(),
                                             fill=dict(color=self.change_color(
                                                 df=df_sites,
                                                 mr_reg=reg,
                                                 df_filter="region")),
                                             font=dict(color='black', size=12),
        ...
        fig.update_traces(hoverinfo="all", row=1,
                          hovertemplate="region: %{text}<br>Количество: %{y}шт.<br>Неделя: %{x}")

        return fig

    def show_push_rollout(self, df, options=None):
        year = str(datetime.now().year)[-2:]
        start_year = int(year + "00")
        end_year = int(year + "00") + 100

        # df = self.get_report_push_capacity(path=self.qet_rollout_path())
        if options:
            df = df[df["IP"].str.contains('|'.join(options), na=False)].reset_index(drop=True)

        '''Получение данных по сайтам у которых нет фактической недели постройки'''
        df_sites = df[["macroregion", "region", "Столбец1", "site_rfs_plan_w", "fact RFS"]]
        df_sites = df_sites[
            (df_sites['site_rfs_plan_w'] >= start_year) & (df_sites['site_rfs_plan_w'] < end_year)
            ].fillna(0)
        df_sites = (df_sites[(df_sites['fact RFS'] == 0)])

        df_sites = df_sites.groupby(["macroregion", "region", "site_rfs_plan_w"]).agg(
            {"Столбец1": lambda x: ", ".join(map(str, x))}).reset_index()
        df_sites = df_sites.rename(columns={'Столбец1': 'sites not built', 'site_rfs_plan_w': 'plan week'}).sort_values(
            "plan week")

        df_plan = df[["macroregion", "region", "site_rfs_plan_w", "IP"]]
        df_fact = df[["macroregion", "region", "fact RFS", "IP"]]

        df_plan = df_plan.dropna(axis='index', how='any', subset=['region'])
        df_fact = df_fact.dropna(axis='index', how='any', subset=['region'])

        df_plan_ip = df_plan[["IP", "region", "site_rfs_plan_w"]].value_counts().reset_index()
        df_plan = df_plan[["macroregion", "region", "site_rfs_plan_w"]].value_counts().reset_index()

        df_plan_ip.rename(columns={0: 'Количество, шт.', 'site_rfs_plan_w': 'Site RFS'}, inplace=True)
        df_plan.rename(columns={0: 'Количество, шт.', 'site_rfs_plan_w': 'Site RFS'}, inplace=True)

        df_fact_ip = df_fact[["IP", "region", "fact RFS"]].value_counts().reset_index()
        df_fact = df_fact[["macroregion", "region", "fact RFS"]].value_counts().reset_index()

        df_fact_ip.rename(columns={0: 'Количество, шт.', 'fact RFS': 'Site RFS'}, inplace=True)
        df_fact.rename(columns={0: 'Количество, шт.', 'fact RFS': 'Site RFS'}, inplace=True)

        df_plan = df_plan[(df_plan['Site RFS'] >= start_year) & (df_plan['Site RFS'] < end_year)].sort_values("region")
        df_fact = df_fact[(df_fact['Site RFS'] >= start_year) & (df_fact['Site RFS'] < end_year)].sort_values("region")
        df_plan_ip = df_plan_ip[(df_plan_ip['Site RFS'] >= start_year) & (df_plan_ip['Site RFS'] < end_year)]
        df_fact_ip = df_fact_ip[(df_fact_ip['Site RFS'] >= start_year) & (df_fact_ip['Site RFS'] < end_year)]

        df_plan["Статус"] = "План"
        df_plan_ip["Статус"] = "План"
        df_fact["Статус"] = "Факт"
        df_fact_ip["Статус"] = "Факт"

        mr_list = df_plan["macroregion"].ffill().unique().tolist()
        reg_list = df_plan["region"].ffill().unique().tolist()
        ip_list = df["IP"].ffill().unique().tolist()
        mr_list.sort()
        reg_list.sort()
        ip_list.sort(key=str, reverse=True)
            ...
            buttons_reg.append(dict(method='restyle',
                                    ...
                                    args=[
                                        {'y': [(df_plan[df_plan["region"] == reg])['Количество, шт.'],
                                         ...
                                         'cells': dict(
                                             values=(df_sites[df_sites["region"] == reg]).transpose().values.tolist(),
                                             fill=dict(color=self.change_color(df=df_sites,
                                                                               mr_reg=reg,
                                                                               df_filter="region")),
                                             font=dict(color='black', size=12),
                                             align='left')
        ...
        buttons_ip = []
        for ip in ip_list:
            buttons_ip.append(dict(method='restyle',
                                   label=ip,
                                   visible=True,
                                   args=[
                                       {'y': [(df_plan_ip[df_plan_ip["IP"] == ip])['Количество, шт.'],
                                              (df_fact_ip[df_fact_ip["IP"] == ip])['Количество, шт.']],
                                        'x': [(df_plan_ip[df_plan_ip["IP"] == ip])['Site RFS'],
                                              (df_fact_ip[df_fact_ip["IP"] == ip])['Site RFS']],
                                        'text': [(df_plan_ip[df_plan_ip["IP"] == ip])['region'],
                                                 (df_fact_ip[df_fact_ip["IP"] == ip])['region']],
                                        'name': [
                                            f"План - {(df_plan_ip[df_plan_ip['IP'] == ip])['Количество, шт.'].sum()} шт.",
                                            f"Факт - {(df_fact_ip[df_fact_ip['IP'] == ip])['Количество, шт.'].sum()} шт."
                                        ],
                                        },
                                   ],
                                   )
        ...
        fig.update_layout(
            ...
            title="Планируемое количество к строительству объектов sites Rollout",
            xaxis_rangeslider_visible=True,
            xaxis_rangeslider_thickness=0.07,
            xaxis_tickangle=0,
            xaxis=dict(
    ...
    def show_push_capacity_lines(self, df_vault):
        # df_vault = self.get_report_push_capacity_vault(path=self.qet_capacity_path())
        # df = PushCapacity().get_report_push_capacity(path=qet_capacity_path())
        # df = df.drop(np.where(df['status'] == "Rent")[0])
            ...
            buttons_reg.append(dict(method='restyle',
                                    ...
                                    args=[
                                        {'x': [df_plan_rfs.index,
                                         ...
                                         'name': [f'План RFS - {round(df_plan_rfs[reg].max())} шт.',
                                                  f'План RFI - {round(df_plan_rfi[reg].max())} шт.',
                                                  f'Факт RFS - {round(df_fact_rfs[reg].max())} шт.',
                                                  f'Факт RFI - {round(df_fact_rfi[reg].max())} шт.',
                                                  f'Факт design - {round(df_fact_design[reg].max())} шт.'],
                                         'visible': True,
                                         },
                                    ],
                                    )
